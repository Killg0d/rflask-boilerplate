...
class TestTaskService(BaseTestTask):
    ...
    def test_create_task(self) -> None:
        account = AccountService.create_account_by_username_and_password(
            params=CreateAccountByUsernameAndPasswordParams(
                first_name="first_name", last_name="last_name", password="password", username="username6"
            )
        )
        with app.test_request_context():
            ...
            task = TaskService.create_task(
                params=CreateTaskParams(
                    accountId = get_account_by_id.id,
                    title="title",
                    description="description"
                )
            )
        assert task.id != None
        assert task.accountId == get_account_by_id.id
    ...
    def test_get_tasks(self)->None:
        account = AccountService.create_account_by_username_and_password(
            params=CreateAccountByUsernameAndPasswordParams(
                first_name="first_name", last_name="last_name", password="password", username="username7"
            )
        )
        with app.test_request_context():
            get_account_by_id = AccountService.get_account_by_id(params=AccountSearchByIdParams(id=account.id))
    ...
    def test_update_task(self)->None:
        account = AccountService.create_account_by_username_and_password(
            params=CreateAccountByUsernameAndPasswordParams(
                first_name="first_name", last_name="last_name", password="password", username="username8"
            )
        )
        with app.test_request_context():
            get_account_by_id = AccountService.get_account_by_id(params=AccountSearchByIdParams(id=account.id))
    ...
    def test_delete_task(self)->None:
        account = AccountService.create_account_by_username_and_password(
            params=CreateAccountByUsernameAndPasswordParams(
                first_name="first_name", last_name="last_name", password="password", username="username9"
            )
        )
        with app.test_request_context():
            get_account_by_id = AccountService.get_account_by_id(params=AccountSearchByIdParams(id=account.id))
